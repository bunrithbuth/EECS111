...
int main(int argc, char** argv) {
	...
	*partition = 0;
	...
	if (thread_num>0) {
		...
		shd_matrix->A = (int*)malloc(sizeof(int)*matrix_size*matrix_size /* fill here */);
		shd_matrix->B = (int*)malloc(sizeof(int)*matrix_size*matrix_size /* fill here */);

		// allocate shared memory for result		
		shm_2d = shmget(IPC_PRIVATE, sizeof(shd_mem)*1/* fill here */, IPC_CREAT | 0666);
	...
	//	printf("Parent getpid = %d  getppid = %d\n", getpid(), getppid());
		for(i = 0; i < thread_num; i++) {
			if (pid > 0/* fill here */) {			
				...
				{
					//do nothing
					//printf("Child %d pid = %d getpid = %d  getppid = %d\n", i, pid, getpid(), getppid());
				}
			}
		}
		...
		//child process
			...
			// Map output memory
			shd_matrix = (shd_mem*)shared_memory;
			output = (int*)shmat(shm_2d/* fill here */, NULL, 0);
			...
			start = (matrix_size/thread_num)*order   /* fill here */;
			end = start + (matrix_size/thread_num)   /* fill here */;
			
			for (i=start; i<end; i++/* fill here */) {
				for (j=0;j<matrix_size;j++) {
					sum=0;
					for (e=0;e<matrix_size;e++) {
						sum+=shd_matrix->A[matrix_size*i+e/* fill here */]*shd_matrix->B[matrix_size*e+j/* fill here */];
					}
					output[matrix_size*i + j/* fill here */]=sum;
				}
			}
		}
		//parent process
			...
			wait();
			C_parent = (int*)malloc(sizeof(int)*matrix_size*matrix_size/* fill here */);
			...
			for (i=0;i<matrix_size;i++) {
				for (j=0;j<matrix_size;j++) {
					...
					C_parent[matrix_size*i+j/* fill here */]=sum;
				}
			}
